...
TEST(QueueManager, Initalizes) { c2014::QueueManager::GetInstance(); }

TEST(QueueManager, QueueWorks) {
  muan::proto::StackProto<PdpStatus, 512> p;
  c2014::QueueManager::GetInstance().pdp_status_queue().WriteMessage(p);

  auto pdp_status_reader = c2014::QueueManager::GetInstance().pdp_status_queue().MakeReader();
